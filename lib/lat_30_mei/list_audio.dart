...
import 'package:mobile_3/lat_30_mei/ModelAudio.dart';
...
class _PageListAudioState extends State<PageListAudio> {
  ...
  Future<void> _fetchAudioData() async {
    try {
      final response = await http.get(Uri.parse('http://192.168.226.154/latAudio/getAudio.php'));
  ...
  Future<void> _play(int index) async {
    ...
    final audioUrl = 'http://192.168.226.154/latAudio/audio/${_audioList[index].audio}';
    try {
  ...
  void _filterAudioList(String query) {
    setState(() {
      ...
      audio.judul.toLowerCase().contains(query.toLowerCase()) ||
          audio.penyanyi.toLowerCase().contains(query.toLowerCase())
      ).toList();
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Playlist', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
      ),
      body: _isLoading
                ...
                return Card(
                  ...
                  child: ListTile(
                    leading: Image.network(
                      'http://192.168.226.154/latAudio/gambar/${_filteredAudioList[index].gambar}',
                      width: 50,
                      height: 50,
                      fit: BoxFit.cover,
                    ...
                    title: Text(_filteredAudioList[index].judul, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
